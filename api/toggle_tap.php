 ...
 *
 * CORREÇÃO v1.1:
 *   - Removida referência à coluna `nome` que não existe na tabela `tap`
 *   - Substituída por JOIN com `bebidas` para obter o nome da bebida
 *   - Adicionado campo `tap_id` na resposta para identificação no app
 */
...
try {
    ...
    // FIX: A tabela `tap` não possui coluna `nome`.
    // Fazemos JOIN com `bebidas` para obter o nome da bebida como identificador.
    $stmt = $conn->prepare(
        "SELECT t.id, t.status, b.name as bebida_name
         FROM tap t
    ...
    Logger::info($TAG,
        "TAP '{$tap['bebida_name']}' (id={$tap['id']}) alterada para status=$statusLabel"
    );
